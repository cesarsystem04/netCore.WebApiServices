using System;
using System.Collections.Generic;
using System.Text;
using System.Diagnostics;
using System.ServiceModel;
using System.ServiceModel.Channels;
using System.Security.Principal;
using System.IO;
using CARSO.LogLibrary.ServiceDataAccess;
using CARSO.AppService.Provider.Service;
using CARSO.AppService.ServiceDataContainers;
using System.Threading.Tasks;

namespace CARSO.AppService.Provider.Client
{
    public partial class AppServiceClient : ClientBase<IAppService>, IAppService
    {
        public DataResponse GetCodigoByCatalogo(CatalogoTipo TipoCatalogo, EstatusActivo Estatus, ref CodigoList oCodigoListResponse)
        {
            return Channel.GetCodigoByCatalogo(TipoCatalogo, Estatus, ref oCodigoListResponse);
        }

        public DataResponse GetCodigoByKey(int idCodigo, ref Codigo oCodigoResponse)
        {
            return Channel.GetCodigoByKey(idCodigo, ref oCodigoResponse);
        }

        public DataResponse GetCodigoByCatalogoAndDsExtra(CatalogoTipo TipoCatalogo, string dsExtra, ref Codigo oCodigoResponse)
        {
            return Channel.GetCodigoByCatalogoAndDsExtra(TipoCatalogo, dsExtra, ref oCodigoResponse);
        }

        public DataResponse GetCodigoByCatalogoAndRfExterna(CatalogoTipo TipoCatalogo, string rfExterna, ref Codigo oCodigoResponse)
        {
            return Channel.GetCodigoByCatalogoAndRfExterna(TipoCatalogo, rfExterna, ref oCodigoResponse);
        }

    }
}
