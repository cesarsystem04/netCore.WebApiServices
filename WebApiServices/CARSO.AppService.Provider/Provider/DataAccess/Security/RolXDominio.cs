...
namespace CARSO.AppService.ServiceDataAccess
{
    public partial class SecurityStore
    {
        ...
        public static DataResponse AddRolXDominio(CARSO.LogLibrary.ServiceDataAccess.TransactionalContext tx, IDataContainer databag)
        {
            SqlCommand cmd = tx.Connection.CreateCommand();
            cmd.Transaction = tx.Transaction;
            cmd.CommandText = "Security.usp_RolXDominioAdd";
            cmd.CommandType = CommandType.StoredProcedure;
            RolXDominioSetNullParemeters(cmd);
            ...
            if (int.Parse(outError.Value.ToString()) != 0)
            {
                ...
                CARSO.LogLibrary.ServiceOperations.LogOperation.Log("AddRolXDominio", err, ConfigurationManager.AppSettings["AppCode"]);
            }
        ...
        private static void RolXDominioSetNullParemeters(SqlCommand cmd)
        {
        ...
        public static DataResponse GetRolXDominioByKey(CARSO.LogLibrary.ServiceDataAccess.TransactionalContext tx, int codRol, string cvDominio, IDataContainer databag)
        {
            SqlCommand cmd = tx.Connection.CreateCommand();
            cmd.Transaction = tx.Transaction;
            cmd.CommandText = "Security.usp_RolXDominioGetByKey";
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                Exception err = new Exception(string.Format("Error ({0}:{1})", outError.Value.ToString(), outMensaje.Value.ToString()));
                CARSO.LogLibrary.ServiceOperations.LogOperation.Log("GetRolXDominioByKey", err, ConfigurationManager.AppSettings["AppCode"]);
            }
