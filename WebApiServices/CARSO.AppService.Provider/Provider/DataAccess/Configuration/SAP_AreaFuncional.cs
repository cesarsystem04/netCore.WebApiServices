...
namespace CARSO.AppService.ServiceDataAccess
{
    public partial class ConfigurationStore
    {
        ...
        public static DataResponse GetAreaFuncionalAll(CARSO.LogLibrary.ServiceDataAccess.TransactionalContext tx, IDataContainer databag)
        {
            SqlCommand cmd = tx.Connection.CreateCommand();
            cmd.Transaction = tx.Transaction;
            cmd.CommandText = "Configuration.usp_AreaFuncionalGetAll";
            ...
            if (int.Parse(outError.Value.ToString()) != 0)
            {
                ...
                CARSO.LogLibrary.ServiceOperations.LogOperation.Log("GetAreaFuncionalAll", err);
            }
        ...
        public static DataResponse GetAreaFuncionalByKey(CARSO.LogLibrary.ServiceDataAccess.TransactionalContext tx, string codAreaFuncional, IDataContainer databag)
        {
            SqlCommand cmd = tx.Connection.CreateCommand();
            cmd.Transaction = tx.Transaction;
            cmd.CommandText = "Configuration.usp_AreaFuncionalGetByKey";
            cmd.Parameters.AddWithValue("@codAreaFuncional", codAreaFuncional);
            cmd.CommandType = CommandType.StoredProcedure;
            SqlParameter outMensaje = new SqlParameter("@Mensaje", SqlDbType.NVarChar, 1000);
            outMensaje.Direction = ParameterDirection.Output;
            cmd.Parameters.Add(outMensaje);
            ...
            {
                Exception err = new Exception(string.Format("Error ({0}:{1})", outError.Value.ToString(), outMensaje.Value.ToString()));
                CARSO.LogLibrary.ServiceOperations.LogOperation.Log("GetAreaFuncionalByKey", err);
            }
            DataResponse oDataResponse = new DataResponse();
            oDataResponse.NumeroError = int.Parse(outError.Value.ToString());
            oDataResponse.Mensaje = outMensaje.Value.ToString();
