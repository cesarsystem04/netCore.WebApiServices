...
namespace CARSO.AppService.ServiceDataAccess
{
    public partial class ConfigurationStore
    {
        ...
        public static DataResponse GetSociedadAll(CARSO.LogLibrary.ServiceDataAccess.TransactionalContext tx, IDataContainer databag)
        {
            SqlCommand cmd = tx.Connection.CreateCommand();
            cmd.Transaction = tx.Transaction;
            cmd.CommandText = "Configuration.usp_SociedadGetAll";
            ...
            if (int.Parse(outError.Value.ToString()) != 0)
            {
                ...
                CARSO.LogLibrary.ServiceOperations.LogOperation.Log("GetSociedadAll", err);
            }
        ...
        public static DataResponse GetSociedadByKey(CARSO.LogLibrary.ServiceDataAccess.TransactionalContext tx, string codSociedad, IDataContainer databag)
        {
            SqlCommand cmd = tx.Connection.CreateCommand();
            cmd.Transaction = tx.Transaction;
            cmd.CommandText = "Configuration.usp_SociedadGetByKey";
            ...
            {
                Exception err = new Exception(string.Format("Error ({0}:{1})", outError.Value.ToString(), outMensaje.Value.ToString()));
                CARSO.LogLibrary.ServiceOperations.LogOperation.Log("GetSociedadByKey", err);
            }
            DataResponse oDataResponse = new DataResponse();
            oDataResponse.NumeroError = int.Parse(outError.Value.ToString());
            oDataResponse.Mensaje = outMensaje.Value.ToString();
